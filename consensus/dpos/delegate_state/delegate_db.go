...
func (d *DelegateDB) createObject(addr common.Address, nickname string, registerTime uint64) (newobj, prev *delegateObject) {
	...
	newobj = newObject(d, addr, Delegate{Nickname: nickname, RegisterTime: registerTime, Vote: big.NewInt(0)})
	if prev == nil {
...
func (d *DelegateDB) Copy() *DelegateDB {
	...
	for addr := range d.delegateObjectsDirty {
		state.delegateObjects[addr] = d.delegateObjects[addr].deepCopy(state)
...
func (d *DelegateDB) CommitTo(deleteEmptyObjects bool) (root common.Hash, err error) {
	// Finalize any pending changes and merge everything into the tries
	d.IntermediateRoot(deleteEmptyObjects)
	...
	for addr, delegateObject := range d.delegateObjects {
		...
		case isDirty:
			...
			if err := delegateObject.CommitTrie(d.db); err != nil {
